...
OscListener::OscListener(int port, Callback cb)
...
std::string OscListener::readOscString(const uint8_t* d, size_t len,
  ...
  std::string s;
  size_t start = off;
  while (off < len && d[off] != 0) s += static_cast<char>(d[off++]);
...
void OscListener::parseMessage(const uint8_t* d, size_t len) {
  ...
  for (int i = 1; i < static_cast<int>(tags.size()); i++) {
    ...
    if (t == 'f') {
      if (off + 4 > len) return;
      uint32_t r = (static_cast<uint32_t>(d[off]) << 24) |
      ...
      uint32_t r = (static_cast<uint32_t>(d[off]) << 24) |
                   ...
                   static_cast<uint32_t>(d[off + 3]);
      off += 4;
      callback_(path, static_cast<float>(static_cast<int32_t>(r)));
    } else if (t == 's') {
...
void OscListener::parseBundle(const uint8_t* d, size_t len) {
  ...
  size_t off = 16;
  while (off + 4 <= len) {
    uint32_t msz = (static_cast<uint32_t>(d[off]) << 24) |
                   (static_cast<uint32_t>(d[off + 1]) << 16) |
                   (static_cast<uint32_t>(d[off + 2]) << 8) |
                   static_cast<uint32_t>(d[off + 3]);
    off += 4;
...
bool OscQueryServer::start() {
  ...
  httpServer_.Get(".*", [this](const httplib::Request& req,
...
void OscQueryServer::setParameterPaths(const std::vector<std::string>& paths) {
  ...
  for (int i = 0; i < static_cast<int>(parameterPaths_.size()); ++i) {
