...
ShockerHub::ShockerHub(Settings& set) : settings(set), chatbox(set.vrchatHost) {}
...
bool ShockerHub::connectSerial() {
  ...
  if (!settings.serialPort.empty()) {
    bool opened = serial.openDevice(settings.serialPort.c_str(), settings.baudRate) == 1;
    if (!opened) {
      logMsg(
  ...
  if (!settings.lastSerialPort.empty()) {
    bool opened =
        serial.openDevice(settings.lastSerialPort.c_str(), settings.baudRate) == 1;
    if (opened) {
...
void ShockerHub::queueShock(int duration, bool vibrate) {
  {
    ...
    shockQueue.push({duration == -1 ? std::nullopt : std::optional<int>(duration),
                     -1, CurveRange::Full, vibrate});
  }
  queueCV.notify_one();
}
...
void ShockerHub::queueShockUpperHalf(int duration, bool vibrate) {
  {
    std::lock_guard<std::mutex> lock(queueMutex);
    shockQueue.push({duration == -1 ? std::nullopt : std::optional<int>(duration),
                     -1, CurveRange::SecondHalf, vibrate});
  }
  queueCV.notify_one();
}
...
void ShockerHub::queueShockFor(int parameterIndex, int duration, bool vibrate) {
  ...
  if (parameterIndex >= 0 && parameterIndex < static_cast<int>(settings.parameters.size())) {
...
bool ShockerHub::resolveOpenShockApi() {
...

  {
    ...
    if (settings.shockerIDs.empty()) {
      for (auto& s : j["data"]) {
        if (s.contains("id")) settings.pushShockerId(s["id"].get<std::string>());
      }
...
std::string ShockerHub::httpGet(
    const std::string& url, const std::vector<std::string>& extraHeaders) {
  ...
  if (res != CURLE_OK) logMsg("[HTTP] GET failed: {}\n", curl_easy_strerror(res));
...
bool ShockerHub::sendPiShockWs(int durationMs, int strength, int shockerId,
  ...
  nlohmann::json cmd = {
      {"Operation", "PUBLISH"},
      {"PublishCommands",
       {{{"Target", "c" + std::to_string(clientId) + "-ops"}, {"Body", body}}}}};
  ...
  std::string url =
      std::string("wss://broker.pishock.com/v2?Username=") + eu + "&ApiKey=" + ek;
  curl_free(eu);
  curl_free(ek);
  ...
  if (res == CURLE_OK && rlen > 0) {
    ...
    if (!rj.is_discarded() && rj.value("IsError", false)) {
      logMsg("[PiShock] Broker error: {}", rj.value("Message", std::string("?")));
      ok = false;
    }
  }
...
bool ShockerHub::scanForPishock() {
  for (auto& port : serialCandidates()) {
    ...
    if (serial.openDevice(settings.serialPort.c_str(), settings.baudRate) != 1) continue;
    ...
    for (int attempt = 0; attempt < 20; attempt++) {
      ...
      if (response.starts_with("TERMINALINFO: ") &&
        ...
        found = true;
        if (settings.shockerIDs.empty()) {
          auto json = nlohmann::json::parse(response.substr(14), nullptr, false);
...
void ShockerHub::startWorkerThread() {
  ...
  if (!workerThread.joinable()) {
    if (settings.useSerial) logMsg("[ShockerHub] Connected on {}\n", settings.serialPort);
...
void ShockerHub::workerLoop() {
  ...
  while (!stopWorker) {
    ...
    if (settings.cooldownEnabled) {
      ...
      if (remaining > 0) {
        std::string cooldownMsg = fmt::format("On cooldown: {:.1f}s", remaining);
        logMsg("{}\n", cooldownMsg);
    ...
    if (settings.randomOrSeq) {
      lastShockerIndex = (lastShockerIndex + 1) % static_cast<int>(ids.size());
      chosenShocker = ids[lastShockerIndex];
    } else {
      std::uniform_int_distribution<int> idxDist(0, static_cast<int>(ids.size()) - 1);
    ...
    if (parameterIndex >= 0 && parameterIndex < static_cast<int>(settings.parameters.size())) {
      ...
      if (curveIndex >= 0 && curveIndex < static_cast<int>(settings.curves.size())) {
    ...
    durationMs = std::max(100, static_cast<int>(durDist(rng) * 1000));
...
void ShockerHub::afterShockSent(int durationMs, int strength,
  ...
  if (settings.chatboxShockEnabled) {
    chatbox.send(
        fmt::format("\xe2\x9a\xa1 {}% | {:.1f}s", strength, durationMs / 1000.0f));
  }

  std::string notifMsg = fmt::format("{}% | {:.1f}s", strength, durationMs / 1000.0f);
...
double ShockerHub::calcDynamicCooldown() const {
  return calculateDynamicCooldown(settings.baseCooldown, settings.maxCooldown,
                                  settings.cooldownFactor,
                                  shockTimestamps.size());
}
  ...
  try {
    if (settings.usePishock) {
      ...
      auto resp = winHttpRequest(
          "POST", "https://" + serverUrl + "/2/shockers/control", payload.dump(),
          {"openshocktoken: " + settings.openshockApiToken});
