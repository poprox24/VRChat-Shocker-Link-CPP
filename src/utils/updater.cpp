...
void checkAsync() {
  std::thread([]() {
    ...
    try {
      ...
      logMsg("[Update] New version {} found, patch name: {}, downloading...", tag,
             relName);
...
namespace {
size_t curlWriteUpd(void* ptr, size_t sz, size_t n, std::string* out) {
  out->append(static_cast<char*>(ptr), sz * n);
  return sz * n;
}

size_t curlWriteFile(void* ptr, size_t sz, size_t n, FILE* f) {
