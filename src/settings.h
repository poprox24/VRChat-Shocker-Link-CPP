...
class Settings {
 public:
  ...
  Settings() : presets(3), parameters(1) {
    // Initialize with 1 default curve
    curves.push_back(Preset());
    curves[0].name = "Default";
  }

  Settings(const std::string& path) : presets(3), parameters(1) {
    // Initialize with 1 default curve
    curves.push_back(Preset());
    curves[0].name = "Default";
    ...
    try {
      ...
      if (j.contains("parameters") && j["parameters"].is_array()) {
        ...
        for (auto& item : j["parameters"]) {
          ...
          p.range = static_cast<CurveRange>(
              item.value("range", (int)CurveRange::Full));
        ...
        if (!secondShockParameter.empty())
          parameters.push_back(
              {secondShockParameter, 0, CurveRange::SecondHalf});
        if (parameters.empty()) parameters.push_back(Parameter());
      }
