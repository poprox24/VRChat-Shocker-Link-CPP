...
void runUI(Settings& settings, ShockerHub& hub,
  ...
  while (!glfwWindowShouldClose(g_window) && running.load()) {
    ...
    // Curve selector bar
    {
      ...
      auto clampPresetActiveIndex = [](SavedPreset& preset) {
        if (preset.activeCurveIndex < 0) preset.activeCurveIndex = 0;
        if (!preset.curves.empty() &&
            preset.activeCurveIndex >= (int)preset.curves.size())
          preset.activeCurveIndex = (int)preset.curves.size() - 1;
      };

      auto appendCurveToAllPresets = [&](const Preset& curve) {
        settings.curves.push_back(curve);
        for (auto& presetSlot : settings.presets) {
          if (!presetSlot.has_value()) continue;
          auto& preset = *presetSlot;
          if (preset.curves.size() < settings.curves.size()) {
            for (size_t idx = preset.curves.size(); idx < settings.curves.size();
                 ++idx)
              preset.curves.push_back(settings.curves[idx]);
          }
          clampPresetActiveIndex(preset);
        }
      };

      auto remapParametersAfterCurveDelete = [&](int deletedCurveIndex) {
        auto remap = [&](std::vector<Parameter>& params) {
          for (auto& param : params) {
            if (param.curveIndex == deletedCurveIndex)
              param.curveIndex = 0;
            else if (param.curveIndex > deletedCurveIndex)
              param.curveIndex--;

            if (param.curveIndex < 0 ||
                param.curveIndex >= (int)settings.curves.size())
              param.curveIndex = 0;
          }
        };

        remap(settings.parameters);
        remap(stgParameters);
      };

      auto deleteCurveFromAllPresets = [&](int curveIndex) {
        if (curveIndex < 0 || curveIndex >= (int)settings.curves.size()) return;

        auto curvesBeforeDelete = settings.curves;
        settings.curves.erase(settings.curves.begin() + curveIndex);

        for (auto& presetSlot : settings.presets) {
          if (!presetSlot.has_value()) continue;
          auto& preset = *presetSlot;

          if (preset.curves.size() < curvesBeforeDelete.size()) {
            for (size_t idx = preset.curves.size(); idx < curvesBeforeDelete.size();
                 ++idx)
              preset.curves.push_back(curvesBeforeDelete[idx]);
          }

          if (curveIndex < (int)preset.curves.size())
            preset.curves.erase(preset.curves.begin() + curveIndex);

          if (preset.activeCurveIndex == curveIndex)
            preset.activeCurveIndex = 0;
          else if (preset.activeCurveIndex > curveIndex)
            preset.activeCurveIndex--;

          clampPresetActiveIndex(preset);
        }

        remapParametersAfterCurveDelete(curveIndex);
      };
      ...
      for (int i = 0; i < (int)settings.curves.size(); i++) {
        ...
        if (ImGui::BeginPopup(("##curve_menu" + std::to_string(i)).c_str())) {
          ...
          if (ImGui::MenuItem("Clone")) {
            ...
            appendCurveToAllPresets(cloned);
            settings.save(settingsPath);
          }
          if (ImGui::MenuItem("Delete")) {
            saveCurrentCurve();
            deleteCurveFromAllPresets(i);
      ...
      if (ImGui::Button("+", {tabButtonWidth, 0})) {
        ...
        appendCurveToAllPresets(newCurve);
        currentCurveIndex = (int)settings.curves.size() - 1;
